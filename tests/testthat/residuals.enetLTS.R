...
test_that("residuals functions works", {
  ## for gaussian
  set.seed(86)
  ...
  resid.fit1 <- residuals(fit1)
  expect_equal(resid.fit1,fit1$residuals)
  ## for binomial
  ...
  expect_equal(resid.fit2,fit2$residuals)
  ## for multinomial
  n <- 120; p <- 15 
  ...
  resid.fit3 <- residuals(fit3)
  expect_equal(resid.fit3,fit3$residuals)
})
